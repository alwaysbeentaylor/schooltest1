...
// ===== NAVIGATION ACTIVE STATE & LINK HANDLING =====
document.addEventListener('DOMContentLoaded', function() {
  ...
  navLinks.forEach(link => {
    ...
    if (href) {
      ...
      if (linkPage === currentPage || (currentPage === 'index.html' && linkPage === 'index.html') || (currentPage === '' && linkPage === 'index.html')) {
        link.classList.add('active');
      }
    }
  });
  
  // Ensure all HTML page links work correctly
  // This ensures navigation works even if there are any issues with event propagation
  document.querySelectorAll('a[href$=".html"]').forEach(link => {
    const href = link.getAttribute('href');
    if (href && !href.startsWith('#') && !href.startsWith('http')) {
      // Make sure these links navigate correctly
      // The browser's default behavior should handle this, but we ensure it works
